...
const DataManager = {
     ...
     */
    getSessionStats() {
        // Bolt: Single-pass loop to avoid array allocations (.map, .filter, .reduce) and CPU overhead on large datasets
    ...
    getErrorsByTable() {
        ...
        // Bolt: Single-pass loop to avoid array allocations (.filter) and CPU overhead on large datasets
    ...
    getResponseTimeDistribution() {
        ...
        // Bolt: Single-pass loop to avoid array allocations (.map) and spread operator with Math.max on large datasets
        let maxFoundTime = 0;
        for (let i = 0; i < len; i++) {
    ...
    getAccuracyDistribution() {
        // Bolt: Single-pass loop to avoid array allocations (two .filter calls) and CPU overhead on large datasets
        let correct = 0;
        let wrong = 0;
        const len = this.history.length;
