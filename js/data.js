...
const DataManager = {
     ...
     * ⚡ BOLT OPTIMIZATION: Replaced chained array methods (.filter, .map, .reduce)
     * with a single O(n) pass to significantly reduce CPU overhead and memory allocation.
     */
    getSessionStats() {
     ...
     * ⚡ BOLT OPTIMIZATION: Replaced .filter().forEach() with a single loop
     ...
     * ⚡ BOLT OPTIMIZATION: Avoids Math.max(...array) which throws 'Maximum call stack size exceeded'
     * on large arrays, and removes intermediate .map() allocation.
     */
     ...
     * ⚡ BOLT OPTIMIZATION: Replaced two O(n) .filter() passes with a single O(n) loop.
     */
