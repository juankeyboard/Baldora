...
const DataManager = {
    ...
    getSessionStats() {
        ...
        // ⚡ Bolt: Single pass for loop avoids intermediate array creation (filter, map) and reduces CPU overhead
        for (let i = 0; i < total; i++) {
            const a = this.sessionData[i];
            if (a.is_correct === 1) {
                correct++;
            }
            sumTime += a.response_time;
        }
    ...
    getErrorsByTable() {
        ...
        // ⚡ Bolt: Single pass for loop avoids filter() allocating intermediate arrays
    ...
    getTopErrors(limit = 5) {
        ...
        // ⚡ Bolt: Single pass for loop avoids filter() memory allocation
        const len = this.history.length;
        for (let i = 0; i < len; i++) {
            const a = this.history[i];
            if (a.is_correct === 0) {
    ...
    getResponseTimeDistribution() {
        ...
        let maxTimeVal = 0;
        // ⚡ Bolt: Map to times while also finding maxTime in one pass without spread operator to prevent call stack exceeded
        ...
        for (let i = 0; i < len; i++) {
            const cappedTime = Math.min(times[i], maxTime);
    ...
    getAccuracyDistribution() {
        ...
        // ⚡ Bolt: Single pass for loop avoids multiple filter() passes on history array
        const len = this.history.length;
        for (let i = 0; i < len; i++) {
