...
const DataManager = {
    ...
    getResponseTimeDistribution() {
        ...
        // Single pass to find max time without exceeding call stack
        ...
        maxTime = Math.min(maxTime, maxLimit);

        const numBins = Math.floor(maxTime / binSize) + 1;
        const binCounts = new Array(numBins).fill(0);

        // Single pass to bucket times into integer-indexed array
        for (let i = 0; i < this.history.length; i++) {
            const t = this.history[i].response_time;
        ...
        // Generate labels only at the end
