...
const DataManager = {
    ...
    getSessionStats() {
        ...
        // ⚡ Bolt: Single pass loop to avoid .filter().map().reduce() intermediate arrays
    ...
    getErrorsByTable() {
        ...
        // ⚡ Bolt: Contar errores con single-pass loop en lugar de .filter().forEach()
    ...
    getTopErrors(limit = 5) {
        ...
        // ⚡ Bolt: Single-pass loop en lugar de .filter().forEach()
        const len = this.history.length;
        for (let i = 0; i < len; i++) {
            const a = this.history[i];
            if (a.is_correct === 0) {
    ...
    getResponseTimeDistribution() {
        ...
        // ⚡ Bolt: Find max time with a single-pass loop instead of Math.max(...times)
        // to avoid "Maximum call stack size exceeded" for large arrays
        let maxFoundTime = 0;
        for (let i = 0; i < len; i++) {
        ...
        // ⚡ Bolt: Integer-indexed arrays instead of string keys in the loop
        ...
        // ⚡ Bolt: Map labels once after bucketing
    ...
    getAccuracyDistribution() {
        ...
        // ⚡ Bolt: Single pass loop en lugar de .filter().length repetido
        const len = this.history.length;
        for (let i = 0; i < len; i++) {
