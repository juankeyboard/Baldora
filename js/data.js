...
const DataManager = {
    ...
    getSessionStats() {
        ...
        // ⚡ Bolt: un solo recorrido para calcular estadísticas en lugar de filter(), map() y reduce()
    ...
    getErrorsByTable() {
        ...
        // Contar errores
        // ⚡ Bolt: un solo recorrido en lugar de encadenar filter().forEach()
    ...
    getResponseTimeDistribution() {
        ...
        // ⚡ Bolt: un solo recorrido para evitar crear array intermedio de tiempos y eludir
        // Math.max(...times) que falla por límite de call stack size en arreglos grandes
        let actualMaxTime = 0;
        for (let i = 0; i < this.history.length; i++) {
            if (this.history[i].response_time > actualMaxTime) {
                actualMaxTime = this.history[i].response_time;
            }
        }

        const maxTime = Math.min(actualMaxTime, 10000); // Cap at 10s
        const bins = {};
        ...
        // ⚡ Bolt: agrupar valores sin usar forEach() en array intermedio
        for (let i = 0; i < this.history.length; i++) {
    ...
    getAccuracyDistribution() {
        ...
        // ⚡ Bolt: un solo recorrido en lugar de dos llamadas a filter()
        for (let i = 0; i < this.history.length; i++) {
