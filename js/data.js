...
const DataManager = {
     ...
     * ⚡ Bolt Optimization: Replace Math.max(...times) with single pass to prevent
     * "Maximum call stack size exceeded" on arrays > 100k items. Also uses integer
     * arrays for fast binning instead of slow object keys and string interpolation.
     */
    getResponseTimeDistribution() {
        ...
        const binSize = 500;
        const absoluteMaxTime = 10000; // Cap at 10s

        // Single pass to find maxTime avoiding maximum call stack size exceeded
        let maxFound = 0;
        ...
        const maxTime = Math.min(maxFound, absoluteMaxTime);
        const numBins = Math.floor(maxTime / binSize) + 1;

        // Use pre-allocated integer-indexed array for faster binning
        ...
        // Map to string labels after loop to avoid string operations in tight loop
