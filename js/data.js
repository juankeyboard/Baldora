...
const DataManager = {
    ...
    getSessionStats() {
        ...
        // Optimization: Single-pass for loop to avoid intermediate array allocations
        // from chained methods like .filter(), .map(), and .reduce()
    ...
    getErrorsByTable() {
        ...
        // Contar errores
        // Optimization: Single-pass loop to avoid intermediate array allocation from .filter()
    ...
    getResponseTimeDistribution() {
        ...
        // Optimization: Calculate maxTime in a single pass without using .map()
        // and without using Math.max(...times) which can cause stack overflow on large arrays.
        let maxFoundTime = 0;
        for (let i = 0; i < historyLen; i++) {
            if (this.history[i].response_time > maxFoundTime) {
                maxFoundTime = this.history[i].response_time;
            }
        }
        ...
        // Optimization: Single-pass over this.history to populate bins
        for (let i = 0; i < historyLen; i++) {
    ...
    getAccuracyDistribution() {
        ...
        // Optimization: Single-pass loop to avoid two separate array allocations from .filter()
        const historyLen = this.history.length;
        for (let i = 0; i < historyLen; i++) {
