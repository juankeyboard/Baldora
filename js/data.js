...
const DataManager = {
     ...
     * ⚡ Bolt: Optimizado con un único bucle para evitar múltiples iteraciones y asignaciones de memoria
     */
    getSessionStats() {
     ...
     * ⚡ Bolt: Eliminado map() y spread en Math.max(...) para evitar desbordamientos de stack con arrays grandes
     */
    getResponseTimeDistribution() {
        ...
        for (let i = 0; i < this.history.length; i++) {
            if (this.history[i].response_time > maxResponseTime) {
                maxResponseTime = this.history[i].response_time;
            }
        }

        // Crear bins de 500ms
        const binSize = 500;
        const maxTime = Math.min(maxResponseTime, 10000); // Cap at 10s
        const bins = {};
     ...
     * ⚡ Bolt: Optimizado con un único bucle para evitar múltiples iteraciones con filter()
     */
