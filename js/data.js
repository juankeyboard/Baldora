...
const DataManager = {
     ...
     */
    getSessionStats() {
        // Bolt: Optimized to use a single pass loop to avoid creating intermediate arrays (filter, map) and reducing overall overhead.
     ...
     */
    getErrorsByTable() {
        // Bolt: Optimized to avoid filter + forEach chain which loops twice and creates an intermediate array.
        const errors = {};
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (item.is_correct === 0) {
                errors[item.factor_a]++;
                errors[item.factor_b]++;
            }
        }
    ...
    getTopErrors(limit = 5) {
        // Bolt: Single pass processing instead of filter+forEach to prevent unnecessary memory allocations.
        const errorCounts = {};
    ...
    getResponseTimeDistribution() {
        // Bolt: Avoid Math.max(...array) which can exceed max call stack size on large histories. Also avoiding map().
        const len = this.history.length;
        if (len === 0) {
        ...
        // Cap at 10s
        maxTime = Math.min(maxTime, 10000);
    ...
    getAccuracyDistribution() {
        // Bolt: Optimized to calculate counts in a single O(N) pass, saving memory and CPU by skipping two filter operations.
        let correct = 0;
        let wrong = 0;
