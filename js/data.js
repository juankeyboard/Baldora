...
const DataManager = {
    ...
    getResponseTimeDistribution() {
        ...
        // ⚡ Bolt: Single-pass loop to find maxTime avoids 'Maximum call stack size exceeded'
        // with Math.max(...array) on large datasets (>100k) and reduces array allocations
        let maxTime = 0;
        ...
        // ⚡ Bolt: Using integer-indexed array for counting instead of string-keyed object
        // is significantly faster and avoids string interpolation inside the hot loop
        const binCounts = new Array(numBins).fill(0);

        for (let i = 0; i < historyLength; i++) {
            const t = this.history[i].response_time;
            const cappedTime = t > 10000 ? 10000 : t;
            let binIndex = Math.floor(cappedTime / binSize);

            // Ensure we don't go out of bounds if floating point math is weird
            if (binIndex >= numBins) {
                binIndex = numBins - 1;
            }
